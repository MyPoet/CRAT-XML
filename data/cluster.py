...
if __name__ == '__main__':
    ...
    if dataset == '670k':
        mlb = build_tree_by_level('./Amazon-670K/train_v1.txt', 
                                  './Amazon-670K/train_labels.txt',
                                  1e-4, 100, [], './Amazon-670K/label_group'+args.id)
        groups = np.load(f'./Amazon-670K/label_group{args.id}-last.npy', allow_pickle=True)
        new_group = []
        for group in groups:
            new_group.append([mlb.classes_[i] for i in group])
        np.save(f'./Amazon-670K/label_group{args.id}.npy', np.array(new_group))
    elif dataset == '500k':
        mlb = build_tree_by_level('./Wiki-500K/train.txt', 
                                  './Wiki-500K/train_labels.txt',
                                  1e-4, 100, [], './Wiki-500K/groups')
        groups = np.load(f'./Wiki-500K/groups-last{args.id}.npy', allow_pickle=True)
        new_group = []
        for group in groups:
            new_group.append([mlb.classes_[i] for i in group])
        np.save(f'./Wiki-500K/label_group{args.id}.npy', np.array(new_group))
